...
#
    10|# Only used by the service wrappers. The CLI profile publishes no port and does not call it.
set -euo pipefail
...
if [[ "${CROPPER_ALLOW_PLAINTEXT:-}" == "1" ]]; then
    20|  echo "Warning: publishing on $bind with CROPPER_ALLOW_PLAINTEXT=1." >&2
...
Choose one:
  - Leave CROPPER_BIND_ADDRESS unset (or 127.0.0.1) and reach the API over
    'brev port-forward', which tunnels it over SSH. This is the shipped default.
